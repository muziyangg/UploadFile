...
createApp({
  setup() {
      ...
      try {
        ...
        try {
            ...
            if (result.success) {
                ...
                // 调用fileRecordManager记录成功上传的文件
                if (window.fileRecordManager && typeof window.fileRecordManager.recordSuccessfulUploads === 'function') {
                    setTimeout(() => {
                        // 使用result.uploadedFiles创建成功文件的列表，包含上传信息
                        const uploadTime = new Date().toLocaleString('zh-CN');
                        // 确保clientInfo存在
                        const clientInfo = window.clientInfo || { ip: '未知IP', location: '未知地址' };
                        const successfulFiles = result.uploadedFiles.map(uploadedFile => ({
                            name: uploadedFile.name,
                            size: uploadedFile.size,
                            status: 'success',
                            time: uploadTime,
                            uploader: uploadedFile.uploader,
                            uploadAddress: uploadedFile.location || '未知地址',
                            ip: uploadedFile.ipAddress || '未知IP'
                        }));
                        window.fileRecordManager.recordSuccessfulUploads(successfulFiles);
                    }, 100);
                }
    ...
    onMounted(() => {
      ...
      // 添加密码框回车事件监听
      document.getElementById('uploadPassword').addEventListener('keydown', async (e) => {
        if (e.key === 'Enter' && uploadPassword.value) {
          // 验证是否为特别密码
            if (window.dealMD && window.dealMD.validateSpecialPassword(uploadPassword.value)) {
              showMessage('密码验证成功，正在获取文件信息...', 'success');
              ...
              // 显示搜索区域和统计iframe
              if (window.dealMD && window.dealMD.showSearchAndStatisticsArea) {
                window.dealMD.showSearchAndStatisticsArea();
              }
              
              // 异步获取文件信息
              try {
                if (window.dealMD && window.dealMD.fetchFileInfoFromMarkdown) {
                  await window.dealMD.fetchFileInfoFromMarkdown();
                  showMessage('文件信息获取成功，已缓存到本地', 'success');
                }
              } catch (error) {
                showMessage('获取文件信息时出错，请稍后重试', 'error');
